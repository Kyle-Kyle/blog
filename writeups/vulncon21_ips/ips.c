...
int alloc_storage(unsigned int priority, char *data) {
  ...
  if(last_allocated_idx >= 0 && !(chunks[last_allocated_idx]->next)) { //null-deref
...
int remove_storage(int idx) {
  ...
  kfree(chunks[idx]);// race condition, not exploitable
  chunks[idx] = NULL;
...
int edit_storage(int idx, char *data) {
  if((idx = check_idx(idx)) < 0); // WTF is this?????
  if(chunks[idx] == NULL) return -1;
...
int copy_storage(int idx) {
  ...
  int target_idx = get_idx();// negative indexing
...
SYSCALL_DEFINE2(ips, int, choice, userdata *, udata) {
  char data[114] = {0};
  if(udata->data && strlen(udata->data) < 115) { // double-fetch
    if(copy_from_user(data, udata->data, strlen(udata->data))) return -1;// not NULL-terminated, potential OOB later => nope, NULL-terminated by canary
  } 
  switch(choice) {
